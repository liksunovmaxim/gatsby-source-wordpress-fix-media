...
const replaceNodeHtmlImages = async ({
  ...
  if (imageUrlMatches.length && imgTagMatches.length) {
    ...
    const cheerioImages = imgTagMatches.map(getCheerioImgFromMatch).filter((el) => {
      if (!el?.cheerioImg?.attribs?.src) {
        return false;
      }

      return (0, _validUrl.isWebUri)(el?.cheerioImg?.attribs.src);
    });
